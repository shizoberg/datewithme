...
export default function CommunityPage() {
  ...
  useEffect(() => {
    document.title = 'Mekan Ekle — getdatewith.me Topluluğu'
    api.get('/api/venues/suggest?city=İstanbul')
      .then(r => {
        // rough count — just show 75+
        setVenueCount(75)
      })
      .catch(() => setVenueCount(75))
  }, [])
  ...
  const inp: React.CSSProperties = {
    ...
    border: '1.5px solid #2A2A2A', background: '#111', color: '#fff',
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0D0D0D', color: '#fff', fontFamily: 'Syne, sans-serif' }}>
      ...
      {/* NAV */}
      <nav style={{ borderBottom: '1px solid #1A1A1A', padding: '16px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Link to="/" style={{ fontWeight: 800, fontSize: '18px', color: '#00F680', textDecoration: 'none', letterSpacing: '-0.5px' }}>
          getdatewith.me
        </Link>
        <Link to="/" style={{ color: '#666', fontSize: '14px', textDecoration: 'none' }}>← Ana Sayfa</Link>
      </nav>

      {/* HERO */}
      <section style={{ textAlign: 'center', padding: '72px 24px 56px', maxWidth: '640px', margin: '0 auto' }}>
        <div style={{ display: 'inline-block', background: '#0A1F14', border: '1px solid #00F68040', borderRadius: '9999px', padding: '5px 16px', fontSize: '12px', color: '#00F680', letterSpacing: '2px', fontWeight: 700, marginBottom: '28px' }}>
          🌿 Community Built
        </div>
        <h1 style={{ fontSize: 'clamp(32px, 7vw, 52px)', fontWeight: 800, lineHeight: 1.1, letterSpacing: '-1px', marginBottom: '20px' }}>
          Bir yerini biliyorsun,<br />
          <span style={{ color: '#00F680' }}>paylaş.</span>
        </h1>
        <p style={{ color: '#888', fontSize: '16px', lineHeight: 1.7 }}>
          getdatewith.me'ye katkıda bulun.<br />
          Şehrindeki en iyi mekanları ekle, herkes faydalansın.
        </p>
      </section>

      {/* STATS */}
      <div style={{ display: 'flex', justifyContent: 'center', gap: '16px', padding: '0 24px 56px', flexWrap: 'wrap' }}>
        {[
          { value: `${venueCount ?? 75}+`, label: 'Mekan' },
          { value: '3',  label: 'Şehir' },
          { value: '↑',  label: 'Sizin katkınızla büyüyor' },
        ].map(s => (
          <div key={s.label} style={{ textAlign: 'center', background: '#111', border: '1px solid #1E1E1E', borderRadius: '16px', padding: '20px 28px', minWidth: '120px' }}>
            <p style={{ fontSize: '28px', fontWeight: 800, color: '#00F680', marginBottom: '4px' }}>{s.value}</p>
      ...
      <div style={{ maxWidth: '640px', margin: '0 auto', padding: '0 24px 80px' }}>
        <div style={{ background: '#111', border: '1px solid #1E1E1E', borderRadius: '20px', padding: '32px' }}>
          <h2 style={{ fontSize: '22px', fontWeight: 800, marginBottom: '6px' }}>Mekanını Ekle</h2>
          <p style={{ color: '#666', fontSize: '14px', marginBottom: '28px' }}>Onaylandıktan sonra tüm kullanıcılara önerilecek.</p>

          {success ? (
            <div style={{ textAlign: 'center', padding: '32px 0' }}>
              <div style={{ background: '#0A1F14', border: '1px solid #00F68040', borderRadius: '14px', padding: '28px', marginBottom: '20px' }}>
              ...
              <button onClick={reset}
                style={{ background: '#00F680', color: '#000', border: 'none', borderRadius: '9999px', padding: '12px 24px', fontWeight: 700, fontSize: '14px', cursor: 'pointer', fontFamily: 'inherit' }}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              {error && (
                <div style={{ background: '#2D1010', border: '1px solid #EF444440', borderRadius: '10px', padding: '12px 16px', marginBottom: '16px', color: '#EF4444', fontSize: '14px' }}>
                  {error}
                </div>
              )}

              <button type="submit" disabled={submitting || !form.name.trim() || !form.city.trim() || !form.district.trim()}
                style={{ width: '100%', padding: '14px', background: submitting || !form.name.trim() || !form.city.trim() || !form.district.trim() ? '#1A1A1A' : '#00F680', color: submitting || !form.name.trim() || !form.city.trim() || !form.district.trim() ? '#555' : '#000', border: 'none', borderRadius: '9999px', fontWeight: 800, fontSize: '16px', cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.15s' }}>
        ...
        <div style={{ marginTop: '48px' }}>
          <h3 style={{ fontSize: '18px', fontWeight: 800, marginBottom: '20px', textAlign: 'center' }}>Nasıl çalışır?</h3>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '14px' }}>
            {[
              ...
              <div key={i} style={{ background: '#111', border: '1px solid #1E1E1E', borderRadius: '14px', padding: '20px 16px', textAlign: 'center' }}>
        ...
        <p style={{ textAlign: 'center', color: '#444', fontSize: '12px', lineHeight: 1.7, marginTop: '32px' }}>
          ...
          Ticari olmayan, gerçek mekan önerileri kabul edilir.<br />
          Spam ve sahte içerikler reddedilir.
        </p>
      </div>
    </div>
  )
